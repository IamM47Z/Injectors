...
void PEParser::map( char* new_ptr_base_address )
{
	...
	for ( auto i = 0; i < this->get_number_of_sections( ); i++, ptr_section_header++ )
	{
		...
		memcpy( new_ptr_base_address + ptr_section_header->VirtualAddress,
				m_pBaseAddress + ptr_section_header->PointerToRawData,
				ptr_section_header->SizeOfRawData );
	}

	memcpy( new_ptr_base_address, m_pBaseAddress, this->get_file_headers_size( ) );

	m_pBaseAddress = new_ptr_base_address;
}
